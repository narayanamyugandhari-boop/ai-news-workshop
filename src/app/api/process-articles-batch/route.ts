  ...
  for (const source of TECH_SOURCES) {
    try {
      ...
      const response = await fetch(
        `https://newsapi.org/v2/everything?sources=${source}&language=en&sortBy=publishedAt&pageSize=3&apiKey=${NEWS_API_KEY}`
      )
...
export async function POST() {
  try {
    console.log('🚀 Starting batch news processing pipeline...')
    ...
    console.log(`📊 Processing ${rawArticles.length} articles with rate limiting...`)
    
    // Step 2: Process each article with AI (with delays)
    ...
    return NextResponse.json({
      success: true,
      message: 'Batch news processing pipeline completed successfully',
      results: {
    ...
    return NextResponse.json({
      success: false,
      message: 'Batch news processing pipeline failed',
      error: error instanceof Error ? error.message : 'Unknown error',
      timestamp: new Date().toISOString()
    }, { status: 500 })
  }
